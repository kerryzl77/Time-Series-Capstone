            ...
            for batch_tuple in tqdm(vali_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                      if self.args.model == 'DeepAR':
                        dec_out = self.model(batch_given, batch_x, batch_mark, meta_x=batch_meta_x, batch_y=batch_y)
                      else:
    ...
    def test_base(self, test_data, test_loader, setting, global_step):
        ...
        with torch.no_grad():
            for batch_tuple in tqdm(test_loader):
                ...
                batch_y = batch_tuple[4].float().to(self.device)
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                      if self.args.model == 'DeepAR':
                        output_tuple = self.model(batch_given, batch_x, batch_mark, 
                                               meta_x=batch_meta_x,
                                               batch_y=batch_y,  # Pass batch_y
                ...
                all_time.append(batch_time.cpu())
                all_id.append(batch_id.cpu())
                all_pred.append(dec_out[:, -self.args.pred_len:].cpu().numpy())
                # all_true.append(batch_y[:, -self.args.pred_len:].data.numpy())
                all_true.append(batch_y[:, -self.args.pred_len:].cpu().numpy())

                tgt_loss = self.criterion(dec_out[:, -self.args.pred_len:].cpu(),
                                          batch_y[:, -self.args.pred_len:].cpu()).item()
